...
void Result::train()
{
	...
	// 2、定义网络结构 并初始化权重参数
	GoogLeNet m_net(NUM_CLASS, true);  //NUM_CLASS为数据集类别总数
	...
	for (int now_iter = 0; now_iter < Iter; now_iter++)
	{
		...
		torch::save(m_net, "GoogLeNet_CPP.pt");
	}
}
...
void Result::pred()
{
	torch::jit::script::Module m_net = torch::jit::load("F:/CCCCCProject/AlexNet/Project1/AlexNet.pt", device_type); // 模型参数的路径根据实际情况修改
...
void Result::pred1()
{
	GoogLeNet m_net(NUM_CLASS, false);  //NUM_CLASS为数据集类别总数
	m_net->to(device_type);
	torch::load(m_net, "GoogLeNet.pt");  // 模型参数的路径根据实际情况修改
	cv::Mat img = cv::imread(img_root);  // opencv读取图片
	cv::cvtColor(img, img, cv::COLOR_BGR2RGB);   // BGR—>RGB
	cv::resize(img, img, cv::Size(224, 224));
	// 将opencv读到的图片转成Tensor并且将BGR格式转成RGB格式
