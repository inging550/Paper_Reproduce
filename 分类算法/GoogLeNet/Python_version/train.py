...
# 2、定义网络结构并设置为CUDA
net = GoogleNet(NUM_CLASS=10, init_weight=True)  # NUM_CLASS为当前数据集的类别总数
net.to(device)
...
for epoch in range(num_epochs):
    ...
    if epoch == 2:
        learning_rate = 1E-5  # 第四次迭代时学习率设置为0.0001
    if epoch == 3:
        learning_rate = 1E-6
    ...
    if best_test_loss > validation_loss:
        ...
        torch.save(net.state_dict(), 'GoogLeNet.pth')  # 保存模型参数
