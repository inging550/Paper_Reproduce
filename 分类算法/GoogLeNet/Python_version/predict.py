...
# 初始化一些参数
img_root = "F:/CCCCCProject/DATASET/TRAIN/2/2_00022.bmp"  # 需要预测的图片路径
net = GoogleNet(NUM_CLASS=10, init_weight=True)  # 给神经网络实例化对象
net.load_state_dict(torch.load("GoogLeNet.pth"))  # 导入权重参数
net = net.cuda()
net.eval()
...
class DrawConfusionMatrix:
    ...
    def drawMatrix(self):
        ...
        plt.title("GoogLeNet ConfusionMatrix")  # title
...
class Predict:
    ...
    def result(self):
        img = cv2.imread(self.img_root)  # opencv读取图片
...
if __name__ == "__main__":
    ...
    # test_dataset = datasets.ImageFolder(root='F:/CCCCCProject/DATASET/TEST', transform=transform)
    ...
    #     labels_np = labels.cpu().numpy()  # array([0,5,0,6,2,...],dtype=int64)
