...
void dataSetClc::load_data_from_folder(std::string path, std::string type, std::vector<std::string> &list_images, std::vector<int> &list_labels, int label, int data_num)
{
	list_images.reserve(data_num);
	list_labels.reserve(data_num);
	// 声明变量
	...
	do
	{
		...
		if (fileInfo.attrib&_A_SUBDIR) //是子文件夹
		{
			...
			label++;
			load_data_from_folder(sub_path, type, list_images, list_labels, label, data_num);
