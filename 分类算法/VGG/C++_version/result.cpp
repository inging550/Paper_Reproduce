...
// 构造函数用于初始化一些参数
// 根据GitHub中的说明进行修改
Result::Result() : device_type(torch::kCUDA),img_root("F:/CCCCCProject/DATASET/TEST/3/3_00122.bmp"), NUM_CLASS(10) ,batch_size(16)
{
}
...
{
	// 1、定义数据集
	auto train_dataset = dataSetClc("F:\\CCCCCProject\\DATASET\\TRAIN", ".bmp", 60000).map(torch::data::transforms::Stack<>());   // 数据集自定义 bmp为后缀名（图片的后缀名也可以为其他比如:JPG,PNG等）
	auto test_dataset = dataSetClc("F:\\CCCCCProject\\DATASET\\TEST", ".bmp", 10000).map(torch::data::transforms::Stack<>());
	...
	// 2、定义网络结构 并初始化权重参数
	VGG16 m_VGG(NUM_CLASS, true);  //NUM_CLASS为数据集类别总数
	//AlexNet* m_VGG = new AlexNet(NUM_CLASS, true);
	m_VGG->to(device_type);
	// 打印模型结构
	std::cout << "模型结构如下" << std::endl;
	for (auto& i : m_VGG->named_parameters())
	{
		std::cout << i.key() << std::endl;
	}

	// 3、定义损失函数以及优化器
	torch::optim::SGD optimizer(m_VGG->parameters(), torch::optim::SGDOptions(m_learn_rate[0]));
	...
	for (int now_iter = 0; now_iter < Iter; now_iter++)
	{
		...
		torch::save(m_VGG, "AlexNet_CPP.pt");
	}
}
...
void Result::pred()
{
	torch::jit::script::Module m_VGG = torch::jit::load("F:/CCCCCProject/AlexNet/Project1/AlexNet.pt", device_type); // 模型参数的路径根据实际情况修改
	...
	m_VGG.eval();
	auto o = m_VGG.forward(std::move(inputs));
...
void Result::pred1()
{
	VGG16 m_VGG(NUM_CLASS, false);  //NUM_CLASS为数据集类别总数
	m_VGG->to(device_type);
	torch::load(m_VGG, "AlexNet_CPP.pt");  // 模型参数的路径根据实际情况修改
	cv::Mat img = cv::imread(img_root);  // opencv读取图片
	cv::cvtColor(img, img, cv::COLOR_BGR2RGB);   // BGR—>RGB
	cv::resize(img, img, cv::Size(224, 224));
	// 将opencv读到的图片转成Tensor并且将BGR格式转成RGB格式
	...
	img_tensor = torch::unsqueeze(img_tensor, 0);
	img_tensor = img_tensor.to(torch::kF32).to(device_type).div(255.0);
	prediction = m_VGG->forward(img_tensor);
